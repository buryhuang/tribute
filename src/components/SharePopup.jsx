...
export default function SharePopup({
  ...
  useEffect(
    () =>
      setModifiedLink(
        `${
          shortLink !== null ? shortLink : window.location.origin + "/" + link
        }`
      ) /* `` */,
    []
  );
  ...
  return (
    <>
      <div className="actions">
        ...
        >
          <button type="button" className={`btn-primary ${shareButtonClass}`}>
            <svg
      ...
      {/* Share Dialog */}
      <div
        className={`fixed z-50 inset-0 overflow-y-auto transition duration-150 ease-in-out ${
          isVisible ? "" : "hidden"
        }`}
      >
              ...
              <div className="sm:flex sm:items-start">
                <div className="text-left w-full">
                  ...
                  <span className="header flex flex-row justify-between items-center">
                    <h3 className="text-xl leading-6 font-medium text-dark-700 dark:text-dark-200">
                      Share
                    </h3>
                    <button onClick={(_) => setVisibility(false)}>
                      <svg
                        className="w-6 h-6 text-dark-700 dark:text-dark-300 scale-90 hover:scale-100 transition-transform duration-75"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                  ...
                  <div className="rounded-sm border border-dark-500 mt-7 py-2 px-3 flex flex-row justify-between items-center">
                    ...
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-6 w-6 text-dark-500 hover:text-dark-600 cursor-pointer ${
                          isCopied ? "hidden" : ""
                        }`}
                        fill="none"
                        viewBox="0 0 24 24"
