...
contract("AIArtFractionalNFT", (accounts) => {
  ...
  it("should mint a new AIArt and distribute fees to referenced artists", async () => {
    ...
    const prevBalanceContractOwner = BigInt(
      await web3.eth.getBalance(accounts[0])
    );

    const fee = web3.utils.toWei("1", "ether");
    await aiArtFractionalNFT.mintAIArt(
      artistAddresses,
      ownershipPercentages,
      imageHash,
      { from: accounts[4], value: fee }
    );

    const tokenId = await aiArtFractionalNFT.tokenOfOwnerByIndex(
      accounts[4],
      0
    );
    const artistList = await aiArtFractionalNFT.getArtistList(tokenId);
    const ownershipPercentageList =
      await aiArtFractionalNFT.getOwnershipPercentageList(tokenId);
    ...
    assert.equal(
      ownershipPercentageList.length,
      3,
      "Ownership percentage list length should be 3"
    );
    ...
    const newBalanceContractOwner = BigInt(
      await web3.eth.getBalance(accounts[0])
    );

    assert.equal(
      newBalance1 - prevBalance1,
      web3.utils.toWei("0.3", "ether"),
      "Artist 1 should receive 30% of fees"
    );
    assert.equal(
      newBalance2 - prevBalance2,
      web3.utils.toWei("0.4", "ether"),
      "Artist 2 should receive 40% of fees"
    );
    assert.equal(
      newBalance3 - prevBalance3,
      web3.utils.toWei("0.3", "ether"),
      "Artist 3 should receive 30% of fees"
    );
    assert.equal(
      newBalanceContractOwner - prevBalanceContractOwner,
      web3.utils.toWei("0.1", "ether"),
      "Contract owner should receive 5% of fees"
    );
